	...
	if(mysql_num_rows($r)==1){ 
		...
		if($r1->username==$u && $r1->password==$p)
		{
            ...
            if($r1->usertype=="Faculty")
            {
             ...
             if($name->gender=="Male")
             {
                 $_SESSION["name"]="Mr.".$name->fname." ".$name->lname;  
             }
            else
            {
                $_SESSION["name"]="Mrs.".$name->fname." ".$name->lname;  
            }
            ...
            if($r1)
             {
                if($r1->dob == $d)
                {
                    echo "<script>location.href='http://172.107.33.53/API/pushsms.aspx?loginID=aamirsit&password=Iloveislam97&mobile=9714512858&text=Password:$r->password&senderid=CHPSMS&route_id=4&Unicode=0';</script>";
...
<img src="img/iqra1.svg" width="50" height="50" style="margin:-3px 0 0 -15px" alt="IQRA">
<span>Iqra</span>
</a>
</header>
...
<input type="password" data-bvalidator="required" id="np" name="password" class="form-control input-lg" placeholder="Password">
...
<?php if(!isset($_REQUEST["fgid"])){?>
<a href="login.php?fgid=1" id="forget">Forget Password ?</a>
<?php }?>
</div>
<?php if(isset($_REQUEST["fgid"])){?>
