...
?>
			<div class="table-responsive">
				<table id="example-datatable" class="table table-bordered table-hover">
					...
					<tbody>
						<?php
							$e=mysql_query("select student_tbl.*,student_sem_tbl.* from student_sem_tbl inner join student_tbl where student_tbl.studid=student_sem_tbl.studid AND student_tbl.isdetend=0 AND student_tbl.isleave=0 AND student_sem_tbl.year='".$_SESSION["year"]."' AND student_sem_tbl.sem=".$_REQUEST["sem"]);
				            ...
				            <?php 
                                ...
                                $fail=0;
                                    ...
                                    ?>    
                                        <td class="text-center" <?php if($m1->marks<17){$fail++;?>style="color:red"<?php }?> ><?= $m1->marks;?></td><?php }else if($ex->exname=="Performance"){ echo "<td class='text-center'>".$m1->marks."</td>"; }else{?> <td class="text-center" <?php if($m1->marks<7){$fail++;?>style="color:red"<?php }?> ><?= $m1->marks;?></td><?php } 
