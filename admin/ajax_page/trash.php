                ...
                <div class="panel-collapse" id="collapseOne">
                    ...
                    <ul class="timeline" style="max-height:300px;min-height:70px;">
                        <?php       include("DatabaseFiles/cn.php");
                                    $r=mysql_query("select * from mail_from_tbl where mailfrom=".$_SESSION['lid']." AND isdelete=1 order by mfid DESC");
                                    ...
                                    $r=mysql_query("select mt.*,mf.* from mail_to_tbl mt inner join mail_from_tbl mf where mt.mfid=mf.mfid and  mt.mailto=".$_SESSION['lid']." AND mt.isdelete=1 order by mt.mtid DESC");
                                    while($r1=mysql_fetch_object($r))
                                    {
                                        $frm=mysql_fetch_object(mysql_query("select * from faculty_tbl where fid=".$r1->mailfrom));
                                    ?>
      ...
      $('.draft').load('ajax_page/draftno.php');
       ...
       $('.trash').load('ajax_page/trashno.php');
        $(".link").click(function(){
            var d = $(this).attr('id');
            var mfid = d;
            $("#open").load('ajax_page/trashmsg.php?mfid='+mfid);
        });
        $(".llink").click(function(){
            ...
            var mtid = u[0];
            var mfid = u[1];
            $("#open").load('ajax_page/trashmsg.php?mtid='+mtid+'&mfid='+mfid);
        });
