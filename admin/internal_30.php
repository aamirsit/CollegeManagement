...
if(isset($_REQUEST["excel"]))
    {
        echo "<script>location.href='../faculty/excel-report/internal_30_report.php?subid=".$_REQUEST['subid']."';</script>";
    }
			...
			<div class="block" style="border: none;box-shadow: none;margin:20px 0">
					<div class="input-group" >
						...
						<select name="subid" class="form-control" style="width:200px;">
                            ...
                            if($flag!=0)
                            {   $r=mysql_query("select * from subject_tbl where sem%2=0 AND sname!='Practical'");
							     while($r1=mysql_fetch_object($r)){?>
                                <option value="<?= $r1->subid?>" <?php if(isset($_REQUEST["subid"])){if($r1->subid==$_REQUEST["subid"]){echo "selected";}}?>><?= $r1->sname ?></option>
                                 <?php }}else{
                            $r=mysql_query("select * from subject_tbl where sem%2!=0 AND sname!='Practical'");
							while($r1=mysql_fetch_object($r)){?>
							<option value="<?= $r1->subid?>" <?php if(isset($_REQUEST["subid"])){if($r1->subid==$_REQUEST["subid"]){echo "selected";}}?>><?= $r1->sname ?></option><?php }}?>
						</select>
					</div>
			</div>
		    </div>
			...
			<div class="table-responsive">
				<table id="example-datatable" class="table table-bordered table-hover">
					...
					<tbody class="text-center">
						<?php 
                             $cc=0;
                             $s=mysql_fetch_object(mysql_query("select * from subject_tbl where subid=".$_REQUEST["subid"]));
                             $sem=$s->sem;
							 $r=mysql_query("select student_tbl.*,student_sem_tbl.* from student_tbl inner join student_sem_tbl where student_tbl.studid = student_sem_tbl.studid AND student_tbl.isdetend=0 AND student_tbl.isleave=0 AND student_sem_tbl.sem=$sem AND student_sem_tbl.year='".$_SESSION["year"]."'");
                                
							while($r1=mysql_fetch_object($r)){
                                $c=mysql_query("select count(*) c from attendence_tbl a,student_sem_tbl s where a.sem=s.sem AND s.sem=$sem AND s.year='".$_SESSION["year"]."' group by a.attenddate");
                        ...
                        ?>
								...
								<td><?php 
                                    ...
                                    {
                                        $qq=mysql_query("SELECT * FROM exam_result_tbl ex INNER JOIN student_sem_tbl ss where ex.smid=ss.smid and ss.sem=$sem and ss.year='".$_SESSION["year"]."' and ex.exid=".$q1->exid." group by ex.exid");
                                        if($qq)
                                        {
                                ...
                                <td>
								    <?php 
                                        $q=mysql_query("select a.* from attendence_tbl a,student_sem_tbl s where a.sem=s.sem AND s.sem=$sem AND s.year='".$_SESSION["year"]."' group by a.attenddate");
								...
								<td><?php 
                                     $total=0;
                                    $en=mysql_fetch_object(mysql_query("select * from exam_tbl where exname like 'P%'"));
                                    $m=mysql_query("select * from exam_result_tbl where exid=".$en->exid." and smid=$r1->smid and subid=".$_REQUEST["subid"]);
                                            while($m1=mysql_fetch_object($m))
                                            {
                                                $total=$total+$m1->marks;
			                    ...
			                    <?php if($cc==0){?>
			                    <td <?php if($intotal<11){?>style="background-color:red;color:white"<?php }?>><?= $intotal?></td><?php }else{ echo "<td><b style='color:#B99663;'>Pending...</b></td>";}?></tr>
							<?php 	}
                                    ?>
					</tbody>
				</table>	
