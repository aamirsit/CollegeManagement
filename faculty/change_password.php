...
$name="Change Password";	
		if(isset($_REQUEST['sbtn']))
		{
			...
			if(isset($_SESSION["fid"]))
			{
                ...
                $r1=mysql_fetch_object(mysql_query("select * from ".$table." where fid=".$_SESSION["fid"]));
                if($r1->password == $_REQUEST["cpwd"])
				    {
                        $col=array('password');
                        $val=array($_REQUEST['npwd']);
                        $db->update($table,'fid',$_SESSION["fid"], $col, $val);
                        echo "<script>location.href='change_password.php?success'</script>";
					
                    }
                else
                {
                        //echo "<script>alert('Current Password Did not Matched');</script>";
                        echo "<script>location.href='change_password.php?un'</script>";
                }
            }
				...
				<form method="post"  id="form1">
					...
					<div class="form-group" >
						<div class="input-group">
							...
							<input id="current" type="password" class="form-control" placeholder="Current Password" name="cpwd" data-bvalidator="required">
