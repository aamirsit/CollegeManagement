<?php
		...
		$filename = 'Rankers'; //your file name
        ...
        $objPHPExcel->setActiveSheetIndex(0) 
					...
					->mergeCells('C3:D3');
        $objPHPExcel->setActiveSheetIndex(0) 
					->setCellValue('E3', 'Report : Top 3 Rankers')
					->mergeCells("E3:".$mn."3");
        $objPHPExcel->setActiveSheetIndex(0) 
        ...
        $fail1=0;
                           ...
                           mysql_query("TRUNCATE TABLE dummy");
                            for($j=0;$j<$c2;$j++)
                                {
                                $total=0;
                                $per=0;
                                ...
                                else
                                {
                                    ...
                                    $col=array('did','smid','rollno','name','marks','per');
                                    $val=array(null,$stud[$j],$roll[$j],$studname[$j],$total,round($per));
                                    $db->insert('dummy',$col,$val);
                                    $rank=mysql_query("select * from dummy order by marks DESC LIMIT 3");
                            }
                                    $iinc=5;
                                    while($ranker=mysql_fetch_object($rank))
                                    {
                                           $objPHPExcel->setActiveSheetIndex(0) 
                                                ->setCellValue('A'.$iinc, $ranker->rollno)
                                                ->setCellValue('B'.$iinc, $ranker->name);
                                ...
                                ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                                            ...
                                            for($k=0;$k<$count;$k++)
                                            {  
                                                $m=mysql_query("select * from exam_result_tbl where exid=".$_REQUEST["exid"]." and smid=$ranker->smid and subid=$subid[$k]");
                                                while($m1=mysql_fetch_object($m))
                                                {
                                                //$total=$total+$m1->marks;
                                                    if($subid[$k]==$m1->subid)
                                ...
                                if($cc==0){
                                           ...
                                           $objPHPExcel->setActiveSheetIndex(0) 
                                                        ->setCellValue($chr[$ainc].$iinc,$ranker->marks);
                                ...
                                //Center    
                                           ...
                                           $ainc++;
                                           $objPHPExcel->setActiveSheetIndex(0) 
                                                        ->setCellValue($chr[$ainc].$iinc,$ranker->per.'%');
