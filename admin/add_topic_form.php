...
if(isset($_REQUEST["sbtn"])) {
		...
		if($_REQUEST["h2"]==""){
			...
			$val=array(null,$_SESSION["lid"],$_REQUEST['topic'],date('Y-m-d h:i:s A'));
			$db->insert($table, $col, $val);
			echo "<script>location.href='".$nm."_form.php?success'</script>";

		} else {
			$col=array('fid','topic','gdtdate');
			$val=array($_SESSION["lid"],$_REQUEST['topic'],date('Y-m-d h:i:s A'));
    ...
    <div class="table-responsive">
				<table id="example-datatable" class="table table-bordered table-hover">
					...
					<tbody class="text-center">
						<?php 
							$r=mysql_query("select * from gd_topic_tbl order by gdtid DESC");
