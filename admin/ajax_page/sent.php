...
<h2>Sent Messages</h2><input id="myInput" type="text" placeholder="Search.."/>
    </div>
                ...
                <div class="panel-collapse" id="collapseOne">
                    ...
                    <ul class="timeline" style="max-height:300px;min-height:70px;">
                        <?php       include("DatabaseFiles/cn.php");
                                    $r=mysql_query("select * from mail_from_tbl where mailfrom=".$_SESSION['lid']." AND isdelete=0 AND isdraft=0  order by mfid DESC");
                                    while($r1=mysql_fetch_object($r))
                                    {
                                        $frm=mysql_fetch_object(mysql_query("select * from faculty_tbl where fid=".$r1->mailfrom));
                                    ?>
                                       <li class="" style="filter:<?php if($r1->isread==0){echo 'sepia(100%)';}?>;"><a id="<?= $r1->mfid?>" class="link" style="cursor:pointer;">
                                    ...
                                    <?php } if(empty(mysql_num_rows($r))){echo "<center><h1 style='margin-top:15%;'><i class='icon-share-alt' style='font-size:100px;'></i><br>No Sent Messages here<br>Thanks For Recycling!</h1></center>";}
                                ?>
                  </ul>
                    </div>  
            </div>
      ...
      $('.draft').load('ajax_page/draftno.php');
       ...
       $('.trash').load('ajax_page/trashno.php');
        $(".link").click(function(){
            var d = $(this).attr('id');
            var mfid = d;
            $("#open").load('ajax_page/sentmsg.php?mfid='+ mfid);
        });
